...
$GOSS build-graph -k $K -O $OUTDIR/graph -i $FRAG_1 -i $FRAG_2 -i $SHORTJUMP_1 -i $SHORTJUMP_2 -B 12 -T 8 -v -l $OUTDIR/build-graph.log &
...
$GOSS thread-pairs -G $OUTDIR/graph-t1-p4-b1 -T 8 -i $FRAG_1 -i $FRAG_2 --insert-expected-size 180 --expected-coverage 20 --min-link-count 4 --innies -v -l $OUTDIR/thread-pairs-frag.log &
...
$GOSS thread-pairs -G $OUTDIR/graph-t1-p4-b1 -T 8 --mate-pairs -i $SHORTJUMP_1 -i $SHORTJUMP_2 --insert-expected-size 3500 --expected-coverage 20 --min-link-count 4 -v -l $OUTDIR/thread-pairs-shortjump.log &
...
# $GOSS build-scaffold -G $OUTDIR/graph-t1-p4-b1 -T 8 -i $FRAG_1 -i $FRAG_2 --insert-expected-size 180 --expected-coverage 60 --scaffold-out $OUTDIR/scaf -v -l $OUTDIR/build-scaffold-frag.log

#mkdir -p $OUTDIR/sg_tp
#cp $OUTDIR/graph-t1-p4-b1*supergraph* $OUTDIR/sg_tp

$GOSS build-scaffold -G $OUTDIR/graph-t1-p4-b1 -T 8 --mate-pairs -i $SHORTJUMP_1 -i $SHORTJUMP_2 --insert-expected-size 3500 --expected-coverage 20 --scaffold-out $OUTDIR/scaf -v 2>$OUTDIR/build-scaffold-shortjump.log &
